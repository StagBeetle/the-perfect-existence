...
air.verb = class{
	...
	constructor(c_processFunction, emotions, c_preposition){
...
air.ai = class { //Class
	constructor(){
		...
		air.emotions.forEach(function(emotion){this.emotions[emotion] = 0;}.bind(this));
	...
	passJudgement (input){//gets given an array of words
		...
		air.emotions.forEach(function(emotion){
			let subWord = this.vocab.nouns[subject];
			let objWord = this.vocab.nouns[object];
			let secWord = this.vocab.nouns[secondobject];
			
			let subEmo = subWord ? subWord.getEmotion(emotion) : undefined;
			let objEmo = objWord ? objWord.getEmotion(emotion) : undefined;
			let secEmo = secWord ? secWord.getEmotion(emotion) : undefined;
			
			
			if(subEmo || objEmo || secEmo){
				let valence = this.vocab.verbs[verb].getValence();
				// lg(emotion);
				// lg(valence);
				// lg(subEmo);
				// lg(objEmo);
				// lg(secEmo);
				returnMoods[emotion] = valence * (subEmo||1.0) * (objEmo||1.0) * (secEmo||1.0);
			}
