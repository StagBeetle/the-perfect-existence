...
air.clusters.eatCluster = {
	"subject": function(event){
		...
		let effectOnArgument = object.getValence() * event.get("action").getValence() * object.getTag("edible");//Only consider valence
		effectOnArgument *= 3; //Because there were two variables
...
air.clusters.baseClusterTwo = {//Used for generic two argument verbs.
	"subject": function(event){
		let object = event.get("object");
		let subject = event.get("subject");
